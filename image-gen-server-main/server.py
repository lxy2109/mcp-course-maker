...
# API配置
JIMENG_API_TOKEN = os.getenv("JIMENG_API_TOKEN") # 你登录即梦获得的session_id   
IMG_SAVA_FOLDER = os.getenv("IMG_SAVA_FOLDER")# 图片默认保存路径
...
async def generate_image(prompt: str, file_name: str, save_folder: str, sample_strength: float = 0.5, width: int = 1024, height: int = 1024) -> list[types.TextContent | types.ImageContent | types.EmbeddedResource]:
    ...
    # 检查并创建保存目录，并在其下新建唯一子文件夹
    save_folder = save_folder or IMG_SAVA_FOLDER
    # 提取关键词（用 file_name 去掉后缀即可）
    keyword = os.path.splitext(file_name)[0]
    # 生成时间戳
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    # 生成唯一子文件夹名
    subfolder_name = f"{keyword}_{timestamp}"
    # 拼接最终保存路径
    final_save_folder = os.path.join(save_folder, subfolder_name)
    if not os.path.exists(final_save_folder):
        os.makedirs(final_save_folder)
        logger.info(f"创建保存目录: {final_save_folder}")
    ...
    try:
        ...
        for i, url in enumerate(image_urls):
            # 构造保存路径
            save_path = os.path.join(final_save_folder, file_name)
