...
public abstract class PathBase : MonoBehaviour
{
    ...
    public virtual float StandardizeUnit(float pos, PositionUnits units)
    {
        ...
        if (len < VectorExtensions.Epsilon)
            return 0;
    ...
    public float ToNativePathUnits(float pos, PositionUnits units)
    {
        if (units == PositionUnits.PathUnits)
            return pos;
        if (DistanceCacheSampleStepsPerSegment < 1 || PathLength < VectorExtensions.Epsilon)
            return MinPos;
        if (units == PositionUnits.Normalized)
            pos *= PathLength;
    ...
    public float FromPathNativeUnits(float pos, PositionUnits units)
    {
        if (units == PositionUnits.PathUnits)
            return pos;
        float length = PathLength;
        if (DistanceCacheSampleStepsPerSegment < 1 || length < VectorExtensions.Epsilon)
            return 0;
