...
app.post('/upload', upload.single('file'), (req, res) => {
  ...
  if (!['hdr', 'exr', 'ktx2', 'jpg', 'jpeg', 'png'].includes(ext)) {
    fs.unlinkSync(file.path);
    return res.status(400).send('仅支持HDR/EXR/KTX2/JPG/PNG文件');
  }
  ...
  fileStore[key] = { base64, type };
  res.redirect(`/preview?type=${type}&key=${key}&name=${encodeURIComponent(file.originalname)}`);
});
  ...
  res.send(`
    ...
    <head>
      ...
      <style>
        ...
        #imgbox { width:100vw; height:100vh; display:flex; align-items:center; justify-content:center; }
        #imgbox img { max-width:100vw; max-height:100vh; display:block; }
    ...
    <body>
      ...
      <div id="imgbox" style="display:none;"><img id="previewimg" /></div>
        ...
        fetch("/file/" + key)
          ...
          .then(data => {
            ...
            if(['jpg','png'].includes(type)) {
              ...
              document.getElementById('previewimg').src = `data:image/${type};base64,${data.base64}`;
            } else {
              const scripts = [
                '/hdr.js',
                '/ktx-parse.js',
