(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.KTX2Container = {}));
  ...
  function getBlockByteLength(container) {
    return container.levels[0].levelData.byteLength / getBlockCount(container);
  }
  /**
  ...
  function getBlockCount(container, levelIndex) {
    ...
    for (let i = 0; i < 3; i++) {
      if (pixelDimensions[i] > 0) {
        const dimBlockCount = Math.ceil(Math.floor(pixelDimensions[i] * 2 ** -0) / blockDimensions[i]);
